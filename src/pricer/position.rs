    ...
    fn compute_cashflow_(position: &Position, date: Date) -> f64 {
        position
            ...
            .map(|trade| match trade.way {
                Way::Sell => -1.0 * trade.quantity * trade.price,
                Way::Buy => trade.quantity * trade.price + trade.tax,
            })
            .sum()
    }
...
mod tests {
    ...
    fn compute_position_with_trade() {
        ...
        }
        {
            ...
            check_indicator_(&indicator, 0.0, 0.0, (0.0, 0.0), 0.09968092530108508, true);
            previous_indicators.push(indicator);
        }
    }
    ...
    fn compute_cashflow() {
        let position = make_position_();
        {
            ...
            assert_float_absolute_eq!(cashflow, 302.55, 1e-7);
        }
        {
            let cashflow = PositionIndicator::compute_cashflow_(&position, make_date_(2022, 3, 19));
            assert_float_absolute_eq!(cashflow, 693.55, 1e-7);
        }
        {
        ...
        }
        {
            ...
            assert_float_absolute_eq!(cashflow, 493.55, 1e-7);
        }
        {
            let cashflow = PositionIndicator::compute_cashflow_(&position, make_date_(2022, 3, 22));
            assert_float_absolute_eq!(cashflow, -10.45, 1e-7);
        }
    }
